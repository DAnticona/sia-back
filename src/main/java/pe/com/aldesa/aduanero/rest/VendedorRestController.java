...
import pe.com.aldesa.aduanero.service.VendedorService;
...
public class VendedorRestController {
	...
	public ResponseEntity<?> create(@RequestBody String request) {
		ApiResponse response;
		try {
			response = vendedorService.save(request);
		} catch (ApiException e) {
			logger.error(e.getMessage(), e);
	...
	public ResponseEntity<?> update(@RequestBody String request) {
		ApiResponse response;
		try {
			response = vendedorService.update(request);
		} catch (ApiException e) {
			logger.error(e.getMessage(), e);
			return new ResponseEntity<>(ErrorResponse.of(e.getCode(), e.getMessage(), e.getDetailMessage()), HttpStatus.PRECONDITION_FAILED);
		}
	...
	public ResponseEntity<?> delete(@PathVariable Long id) {
		ApiResponse response;
		try {
			response = vendedorService.delete(id);
		} catch (ApiException e) {
			logger.error(e.getMessage(), e);
			return new ResponseEntity<>(ErrorResponse.of(e.getCode(), e.getMessage(), e.getDetailMessage()), HttpStatus.NOT_FOUND);
		}
